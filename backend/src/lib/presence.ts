...
/*
 * The admin's stop instruction used to be read here, from
 * `listening_status.force_pause_at`. It now lives on the account as a hold
 * with an end — see `holdFor` in lib/moderation.ts — because a stamp on the
 * status row could only be applied once and vanished entirely whenever the
 * listener was between tracks.
 */
